...
# venv собран на этапе сборки и неизменяем: `uv run` (compose `command:`,
# deploy.sh) не должен ничего досинхронизировать на старте. Без этого он
# считал dev-группу дефолтной и на каждом запуске контейнера тянул из сети
# pytest/ruff/ty — сводя на нет `--no-dev` выше и привязывая старт к PyPI.
ENV UV_NO_SYNC=1
