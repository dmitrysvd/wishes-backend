...
- После создания worktree запусти `scripts/worktree-init.sh <путь>` — подтянет симлинки
  `wishes-product` и `.env` из основного чекаута; без них не работают `snapshot.sh` и тесты.
