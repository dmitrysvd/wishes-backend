...
## Тесты
- Держи 100% покрытие для `app/` (проверяется в pre-commit).
- Фикстуры pytest для БД и пользователей; данные чистятся фикстурой `db`.
- Переопределяй зависимости через `app.dependency_overrides`.
- Внешние вызовы (Firebase, VK, Telegram) мокай через `pytest-mock`.
