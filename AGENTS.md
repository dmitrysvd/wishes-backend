...
## Worktrees
Каждая задача — в своём worktree на своей ветке, заводится только через Claude (`EnterWorktree`): хук `WorktreeCreate` подтягивает gitignored-окружение. Голый `git worktree add` не использовать — окружение окажется пустым. Ветки в общей рабочей копии не переключать. После мержа worktree удалить.
