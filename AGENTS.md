...
- **Контракт = интерфейс.** Payload других каналов (пуш, deep link) — структурно в
  `openapi_extra={'x-push-payload': {...JSON-пример `data` + `notification`, тексты...}}`
  на операции-приёмнике; `description` — только про саму ручку. Внутреннее поведение бэка
  без HTTP-следа (крон, триггеры, дедуп, антиспам) в контракт **не входит** и в
  `description`/`x-*` ради аудитора не кладётся (PROTOCOL.md §7/§8): его место — intent,
  docstring и тесты.
