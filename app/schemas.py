...
class WishReadSchema(BaseWishSchema):
    ...
    is_reserved: bool = Field(
        description=(
            'Хотелка кем-то зарезервирована. Явный флаг состояния резерва — '
            'предпочтительный способ его показать; личность дарителя при этом '
            'скрыта, см. `reserved_by_id`.'
        )
    )
    reserved_by_id: UUID | None = Field(
        description=(
            'Кто зарезервировал, но личность раскрывается ТОЛЬКО самому дарителю. '
            'Три состояния: `null` — не зарезервирована; id запрашивающего — '
            'зарезервировал он сам (по нему клиент считает «зарезервировано мной» и '
            'даёт отмену); `00000000-0000-0000-0000-000000000000` — зарезервировал '
            'кто-то другой, личность скрыта. Владельцу списка имя дарителя — спойлер '
            'сюрприза, остальным — чужая PII, поэтому реальный чужой id не отдаётся '
            'никогда. Заглушка не `null` намеренно: клиент, считающий непустое поле '
            'признаком резерва, продолжает работать. Новым клиентам вместо этого '
            'поля — `is_reserved`.'
        )
    )
    image: str | None
