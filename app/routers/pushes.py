...
Payload пуша по складу — `x-push-payload` этой операции; правила фичи (кому,
отбор хотелок, триггеры, дедуп, база «видел») — `x-workflow`. Swagger UI
расширения не показывает — читайте спек.
"""
...
# Правила фичи (кому, отбор, триггеры, дедуп, база «видел») — самодостаточно.
_WORKFLOW = [
    'Кому: только автору хотелки (не резервировавшим, не подписчикам).',
    'Отбор хотелок: активные (не архив), ссылка WB, price_source = shop. Ручная '
    'цена — магазин молчит, пушей нет.',
    'Триггер «подешевело»: цена наблюдения суточного обхода ниже базы «видел» на '
    '≥ 10 %. Рост цены — молчим.',
    'Отметки просмотра нет: клиент ничего не шлёт, GET карточки/списка ничего не '
    'двигает.',
    'База «видел» = цена со скидкой последнего наблюдения с датой ≤ последнего дня '
    'активности юзера (user_activity_day); наблюдений до неё нет — цена хотелки на '
    'момент создания (первое наблюдение с ценой); после пуша — цена из пуша (что '
    'позже).',
    'Триггер «снова в наличии»: предыдущее наблюдение обхода sold_out/gone, новое — '
    'в наличии с ценой; один пуш на возврат; одиночный запрос кнопкой стартом не '
    'считается. Временно выключен (dry-run 2026-09-13: дневное мерцание остатков); '
    'enum и тексты остаются.',
    'Один пуш в календарные сутки UTC на юзера по складу: все сработавшие за обход '
    'хотелки — одним сообщением; перезапуск обхода в те же сутки второго пуша не '
    'даёт. Отправка — в полдень, ночных нет.',
    'Сброс базы «видел» — только при успешной отправке (FCM принял); мёртвый токен '
    'и сбой отправки базу не двигают.',
    'Базу также двигают кнопка «актуальная с WB» (если принесла цену) и включение '
    'группы prices (0012: база = текущая цена карточки; при выключенной группе '
    'события не копятся).',
