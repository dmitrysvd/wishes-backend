...
# Генерик-ошибки описаны здесь один раз (PROTOCOL.md §7); на операциях — только
# коды с семантикой, специфичной для них.
API_DESCRIPTION = """
## Общие ошибки (на любой операции, на операциях не повторяются)

- `401` — нет заголовка `Authorization`, токен битый или истёк. Клиент
  переавторизуется. Исключение — публичные операции (`security: []`).
- `422` — тело/параметры нарушают схему: нет обязательного поля, не тот тип,
  пустая строка при `minLength`, значение вне `enum`. Тело —
  `HTTPValidationError`. Это баг клиента, а не состояние — специальной
  обработки не требует. Если у операции `422` несёт бизнес-правило, не выразимое
  схемой, оно описано на операции.
- `5xx` — сбой сервера; тело без контрактной формы. Клиент показывает генерик
  «попробуйте позже».
"""
